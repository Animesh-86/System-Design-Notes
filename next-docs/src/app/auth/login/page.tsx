...
function LoginPageInner() {
  ...
  useEffect(() => {
    if (session?.user) {
      // If we already have a session (e.g., after OAuth callback), navigate
      // to the original requested page or the dashboard.
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#050506] text-white px-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/[0.02] p-8 shadow-lg">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">System Design Hub</h1>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="mt-6 flex w-full items-center justify-center gap-3 rounded-lg bg-white px-4 py-3 text-sm font-semibold text-slate-950 disabled:opacity-60"
        >
