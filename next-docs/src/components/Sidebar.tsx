...
import { Book, FileText, File, Layers, ChevronRight, Search, Sun, Moon } from 'lucide-react';
...
export function Sidebar({ items }: { items: ContentItem[] }) {
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    lld: false, // Collapsed by default since it has 73 items!
    docx: true,
    pdf: true,
    resource: true,
  });
  ...
  // If a search query is entered, auto-expand matching sections
  const isSearching = searchQuery.trim() !== '';

  return (
    <aside className="w-80 h-screen overflow-hidden border-r border-black/5 dark:border-white/10 bg-white/80 dark:bg-[#09090b]/80 backdrop-blur-xl flex flex-col flex-shrink-0 z-30 transition-colors duration-300">
      {/* Brand Header */}
      <div className="p-6 border-b border-black/5 dark:border-white/5 bg-black/[0.01] dark:bg-black/20">
        <div className="flex items-center justify-between">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-lg bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors shadow-sm"
          ...
          return (
            <div key={type} className="border border-black/5 dark:border-white/5 rounded-xl bg-black/[0.005] dark:bg-white/[0.01] overflow-hidden">
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-3 hover:bg-black/5 dark:hover:bg-white/5 transition-colors duration-200 text-left"
              >
                      ...
                      {typeItems.map(item => {
                        ...
                        return (
                          <Link
                            ...
                            href={`/docs/${item.slug}`}
                            className={clsx(
                              "block px-3 py-2 rounded-lg text-xs transition-all duration-150 relative",
                              isActive
      ...
      {/* Sidebar Footer */}
      <div className="p-4 border-t border-black/5 dark:border-white/5 bg-black/[0.01] dark:bg-black/20 text-center">
        <span className="text-[10px] text-gray-400 dark:text-gray-600 font-mono">
          System Design Hub v1.0.0
        </span>
      </div>
    </aside>
  );
